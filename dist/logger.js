...
var Console = _winston.transports.Console,
    File = _winston.transports.File;


var LOG_FILE_NAME = '.application.log';
var LOG_FILE_PATH = process.env.NODE_ENV === 'production' ? _path2.default.join((0, _os.homedir)(), LOG_FILE_NAME) : _path2.default.join(__dirname, '..', '..', LOG_FILE_NAME);
...
logger.expressMiddleware = function expressMiddleware(req, res, next) {
  ...
  var printExecutionTime = function printExecutionTime() {
    var statusCode = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  ...
  req.on('end', function () {
    return printExecutionTime(res.statusCode);
  });
  req.on('close', function () {
    return printExecutionTime('[closed by user]');
  });
